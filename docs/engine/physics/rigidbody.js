...
export class RigidBody extends Script {
    ...
    fixedUpdate() {
        ...
        for (const collider of this.gameObject.getComponentsInChildren(Collider)) {
            for (const otherCollider of SceneManager.activeScene.findComponents(Collider)) {
                ...
                if (this.gameObject.transform === otherRoot)
                    continue;
                let collision = null;
                if (collider instanceof CircleCollider) {
                    if (otherCollider instanceof CircleCollider) {
                        ...
                        if (sqr_mag < radius * radius) {
                            ...
                            collision = new Collision(otherCollider, normDiff.mul_s(Math.sqrt(sqr_mag) - radius), normDiff.mul_s(otherCollider.gameObject.transform.scale.x), otherRoot.gameObject, collider);
                        }
                    }
                ...
                if (!collider.isTrigger)
                    this.gameObject.transform.position = this.gameObject.transform.position.add(collision.correctionVector);
        ...
        for (let i = 0; i < newCollisions.length; i++) {
            ...
            let duplicate = false;
            for (let j = 0; j < i; j++) {
                if (collision.gameObject === newCollisions[j].gameObject &&
            ...
            for (let j = 0; j < this.collisions.length; j++) {
                if (collision.gameObject === this.collisions[j].gameObject &&
    ...
    circleRectIntersects(circle, rect) {
        ...
        if (dist >= (circleRadius * circleRadius))
            return null;
        return new Collision(rect, diff.normalize().mul_s(circleRadius - Math.sqrt(dist)), closest, rect.gameObject, circle);
    }
}
export class Collision {
    constructor(collider, correctionVector, contactPoint, gameObject, source) {
