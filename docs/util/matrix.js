...
export class Mat3 {
    ...
    mul_vec2(x) { return new Vec2(this.array[0] * x.x + this.array[1] * x.y + this.array[2], this.array[3] * x.x + this.array[4] * x.y + this.array[5]); }
    // Scalar operations
    add_s(x) {
        let temp = [];
        for (let i = 0; i < 9; i++)
    ...
    get_scale() {
        return new Vec2(Math.sqrt(this.array[0] * this.array[0] + this.array[3] * this.array[3]), Math.sqrt(this.array[1] * this.array[1] + this.array[4] * this.array[4]));
    }
    ...
    static create_translation(translation) { return new Mat3([1, 0, translation.x, 0, 1, translation.y, 0, 0, 1]); }
    static create_scale(scale) { return new Mat3([scale.x, 0, 0, 0, scale.y, 0, 0, 0, 1]); }
    static create_rotation(rotation) {
        let c = Math.cos(rotation);
        let s = Math.sin(rotation);
        return new Mat3([c, -s, 0, s, c, 0, 0, 0, 1]);
    }
    static create_transformation(translation, scale, rotation) {
